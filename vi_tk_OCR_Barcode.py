...
from os import getcwd,mkdir,path,system,listdir,walk
...
def read_sql_jobid(jobver,process_department):
...
def read_data_matrix(jobver_id, filename,filename_OCR, identifier):
    global end_filename
    # print(filename,filename_OCR)
    dm_data=decode(cv2.imread(filename))
    sn = open(filename_OCR, 'a')
    if len(dm_data)==0:
        sn.write('')
    else:
        sn.write(dm_data[0][0].decode())
    sn.close()
    end_filename = filename


def read_data_matrix_sample(filename):
    global end_filename
    # print(filename,filename_OCR)
    dm_data=decode(cv2.imread(filename))
    ...
    if len(dm_data)==0:
        ...
        scan_type='data_matrix'

    return scan_type


def ocr_thread(jobver_id, filename,filename_OCR, identifier,TrainingFont):
    """
    OCR 處理線程
    ...
    text = pytesseract.image_to_string(filename, lang=TrainingFont, config="--psm 6")
...
def process_img(img1,S_target,row_count,column_count,jobver_id,i,identifier,frame,TrainingFont,kernel):
    ...
    np_ker=np.ones((kernel,kernel),np.uint8)
    if kernel:
        binary=cv2.erode(binary,np_ker)
    # ##Barcode 不需要過濾圖像
    ...
    for cnt in contours:
        ...
        if ((cv2.contourArea(cnt) > S_target) and (max_xy[0, 0] - min_xy[0, 0]) < row_count and (max_xy[0, 1] - min_xy[0, 1]) < column_count) or ni == 1:  # 計算轮廓面積, 大於100的不處理. 小於100的用才處理.
            continue
    ...
    filename_org = "./images_org/" + jobver_id + img_file + ".png"
    ...
    thread_ocr = threading.Thread(target=ocr_thread, args=(jobver_id, filename, filename_OCR, identifier,TrainingFont))
    # thread_ocr = threading.Thread(target=read_data_matrix, args=(jobver_id, filename, filename_OCR, identifier))
    thread_ocr.start()


def process_img_DM(img1,S_target,row_count,column_count,jobver_id,i,identifier,frame):
    #     # gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    # ret, binary = cv2.threshold(frame, 150, 255, cv2.THRESH_BINARY)      #将图像信息二值化，处理过后的图片只有二种色值(黑0,白255)  ; 大於100的值變為白色(255), 小於等於100的變為黑色(0)
    ret, binary = cv2.threshold(img1, 100, 255,cv2.THRESH_BINARY)  # 将图像信息二值化，处理过后的图片只有二种色值(黑0,白255)  ; 大於100的值變為白色(255), 小於等於100的變為黑色(0)
    # ret, binary = cv2.threshold(img1, 50, 255,cv2.THRESH_BINARY)  # 将图像信息二值化，处理过后的图片只有二种色值(黑0,白255)  ; 大於100的值變為白色(255), 小於等於100的變為黑色(0)
...
def readv(jobver, rotate, identifier, data_type, composing, box,scan_type,kernel):
    """
    讀視頻幀
    ...
    TrainingFont = config['DEFAULT']['TrainingFont']
    # kernel = int(config['DEFAULT']['kernel'])
    # ocr_list=set()
    ...
    jobver_id=read_sql_jobid(jobver, process_department)
    # # 打开文件
    # fo = open("./jobver_list.txt", "a+")
    # # print("文件名为: ", fo.name)
    # fo.seek(0, 0)
    ...
    while (cv2.waitKey(1) & 0xFF) != 27:      #ord('q')
        ...
        try:
            ...
            cv2.imshow("Press <ESC> to Exit Program!", frame)
            if np.count_nonzero(frame)<2000:
            # if np.max(frame)==0 and np.min(frame)==0:
                pass
            else:
                #####加入圖像處理去黑點
                x1, y1 = 4, 4
                ...
                img1[y1:y2, x1:x2] = frame[2:height1-2, 2:width1-2]     #去黑边, 需要再测试
                # print(y1 + 2, y2, x1 + 2, x2)
                # print(2, height1, 2, width1)
                # print(y1 + 2, y2 - 2, x1 + 2, x2 - 2)
                # print(2, height1 - 2, 2, width1 - 2)

                thread_process_img = threading.Thread(target=process_img, args=(img1,S_target,row_count,column_count,jobver_id,i,identifier,frame,TrainingFont,kernel))
    ...
    thread = threading.Thread(target=ocr_thread, args=(jobver_id, filename,filename_OCR, identifier,TrainingFont))
    thread.start()
...
def readv_DM(jobver, rotate, identifier, data_type, composing, box,scan_type):
    """
    讀視頻幀
    ...
    while (cv2.waitKey(1) & 0xFF) != 27:      #ord('q')
        ...
        try:
            ...
            x2, y2 = width1 + 4, height1 + 4
            img1 = np.zeros(((height1 + 8), (width1 + 8),1), np.uint8) + 255  # 建立空白图片.
            # img1[y1 + 2:y2-2, x1 + 2:x2-2] = frame[2:height1-2, 2:width1-2]
            img1[y1 + 2:y2, x1 + 2:x2] = frame[2:height1, 2:width1]


            thread_process_img = threading.Thread(target=process_img_DM, args=(img1,S_target,row_count,column_count,jobver_id,i,identifier,frame))
            thread_process_img.start()
            # txt_file.write(getcwd() + "/images/" + jobver_id + "/img" + str(i) + ".png" + "\n")  # 保存图像文件路径; getcwd() #返回当前工作目录
            # if i % file_count == 0:  # 最后一个images就Close txt文件   2021-11-03
            #     txt_file.close()
    ...
    mvsdk.CameraAlignFree(pFrameBuffer)
    condition_std_char("./images/" + jobver_id + "/img0000001.png")
    thread = threading.Thread(target=read_data_matrix, args=(jobver_id, filename,filename_OCR, identifier,))
    thread.start()
    # print("結束OCR", filename)
    # # 合並txt 檔案
    # copyfile('D:/Xu/python/MV/working.txt', "D:/Xu/python/MV/job/" + jobver_id + "/setting.txt")
...
def check_tag_images():
    max_file = ''
    max_datetime = 1.0
    path1 = "d:/test"
    for root, dirs, files in walk(path1):
        for f in files:
            if getmtime(path1 + "/" + f) > max_datetime:
                max_datetime = getmtime(path1 + "/" + f)
                max_file = path1 + "/" + f

    print(max_file, max_datetime)
    return max_file
...
load_check()
update_std_char()
max_file=check_tag_images()
scan_type=read_data_matrix_sample(max_file)
...
kernel=tkinter.StringVar(myWindow)
...
label1d=tkinter.Label(frame1, text="去白线", justify="left").grid(row=0, column=8)
entry_kernel=tkinter.Entry(frame1, text="", textvariable=kernel, justify="left", width=4).grid(row=0, column=9)
kernel.set('0')
frame1.grid(row=0, column=0)
...
if scan_type=='OCR':
    button2=tkinter.Button(frame2A, text=" 開始編號讀取 ",font=('Arial',28,'bold'), justify="left", command=lambda: readv(entry_jobver.get(),rotate.get(), identifier.get(), print_type.get(),layout_count.get(),box_qty.get(),scan_type,int(kernel.get()))).grid(row=4, column=1)
else:
    button2 = tkinter.Button(frame2A, text=" 開始編號讀取 ", font=('Arial', 28, 'bold'), justify="left",
                             command=lambda: readv_DM(entry_jobver.get(), rotate.get(), identifier.get(), print_type.get(),
                                                   layout_count.get(), box_qty.get(),scan_type)).grid(row=4, column=1)
