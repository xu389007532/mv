...
def main_loop():
	...
	# 相机模式切换成连续采集
	mvsdk.CameraSetTriggerMode(hCamera, 0)		#0表示连续采集模式；1表示软件触发模式；2表示硬件触发模式。
	...
	mvsdk.CameraSetAnalogGainX(hCamera, 5)
	...
	a,b=cap.sResolutionRange.iWidthMax,cap.sResolutionRange.iHeightMax
	# FrameBufferSize = 1280 * 1024
	pImageResolution=mvsdk.CameraCustomizeResolution(hCamera)
	mvsdk.CameraSetImageResolution(hCamera,pImageResolution)

	# 分配RGB buffer，用来存放ISP输出的图像
	# 备注：从相机传输到PC端的是RAW数据，在PC端通过软件ISP转为RGB数据（如果是黑白相机就不需要转换格式，但是ISP还有其它处理，所以也需要分配这个buffer）
	pFrameBuffer = mvsdk.CameraAlignMalloc(FrameBufferSize, 16)
	frams=mvsdk.CameraGetFrameSpeed(hCamera)
	check1=mvsdk.CameraSetFrameSpeed(hCamera,2)
	...
	while (cv2.waitKey(1) & 0xFF) != ord('q'):
		...
		try:
			...
			mvsdk.CameraSaveImage(hCamera, "./test2.bmp", pFrameBuffer, FrameHead, mvsdk.FILE_BMP, 100)
